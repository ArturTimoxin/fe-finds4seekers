...
import UserIcon from "../../public/images/profile2.png";
...
const UserAnswerBlock = ({
  ...
  return (
    <>
      <Feed.Event className="answer-wrap">
        <Feed.Label image={UserIcon} />
        <Feed.Content className="answer-content-wrap">
          <Feed.Summary>
            <Feed.User>{`${firstname} ${lastname}`}</Feed.User>
            <Feed.Date>{createdAt}</Feed.Date>
          </Feed.Summary>
          <Feed.Extra text>{answerText}</Feed.Extra>
          <div>
            <Feed.Meta>Phone: {phone}</Feed.Meta>
          </div>
          <div>
            <Feed.Meta>Email: {email}</Feed.Meta>
          </div>
        </Feed.Content>
        <div className="answer-actions-wrap">
          ...
          />
          <Button onClick={() => onSendContactData(id)}>
