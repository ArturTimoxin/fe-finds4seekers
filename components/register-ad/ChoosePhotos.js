...
const ChoosePhotos = ({ typeAd, photos = [] }) => {
  ...
  const onChangePhoto = (e) => {
    if (e.target.files.length) {
      ...
      if (e.target.name === "subPhotoOne")
        setSubPhotoOne(URL.createObjectURL(e.target.files[0]));
      if (e.target.name === "subPhotoTwo")
        setSubPhotoTwo(URL.createObjectURL(e.target.files[0]));
    } else {
  ...
  const getPhotoImg = (photoData, defaultPhoto = PlusIcon) => {
    if (defaultPhoto !== PlusIcon && !photoData) {
      return <img src={defaultPhoto} alt="plus" className="photo-preview" />;
    }

    if (!photoData) {
      return <img src={defaultPhoto} alt="plus" className="plus-icon" />;
    }

    return <img src={photoData} className="photo-preview" />;
  };

  return (
    <Form.Field>
      <label className="choose-photos-wrap">
        {`Choose photos of your ${
          typeAd === AD_LOST_TYPE_ID ? "lost" : "found"
        }:`}
        <div className="choose-photos-input">
          <label
            htmlFor="main-photo"
            className="main-photo-input choose-photos-wrap"
          >
            {photos[0]
          ...
          <div className="sub-photos-inputs-wrap">
            <label
              htmlFor="sub-photo-one"
              className="sub-photo-input choose-photos-wrap"
            >
              {photos[1]
